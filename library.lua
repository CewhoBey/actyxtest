...
local ESP_SETTINGS = {
    ...
    TeamCheck = false,   -- fixed casing (was Teamcheck)
    WallCheck = false,
    Enabled = false,
    ShowBox = false,
    BoxType = "2D",
...
local function createEsp(player)
    local esp = {
        -- removed duplicate 'tracer' key; keeping only this one
        tracer = create("Line", {
...
local function isPlayerBehindWall(player)
    ...
    -- replaced deprecated FindPartOnRayWithIgnoreList with workspace:Raycast
...
local function removeEsp(player)
    ...
    -- skeletonlines and boxLines are plain tables, not Drawing objects
    for key, val in pairs(esp) do
        if key == "skeletonlines" then
            for _, lineData in ipairs(val) do
                lineData[1]:Remove()
            end
...
-- Safely hides all drawings for an esp entry without touching plain tables
local function hideAllDrawings(esp)
    for key, val in pairs(esp) do
        if key ~= "skeletonlines" and key ~= "boxLines" then
            val.Visible = false
        end
    end
